<!-- resources/views/chargers/create.blade.php -->
@extends('layouts.app')
...
<div class="max-w-5xl mx-auto">
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <div class="flex items-center gap-3">
                <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Tambah Data Charger</h1>
            ...
            <p class="text-sm text-slate-500 mt-1">Isi detail perawatan atau penarikan charger unit.</p>
        </div>
    ...
    <!-- Form Container -->
    <form id="form-charger" action="{{ route('chargers.store') }}" method="POST" class="space-y-6 pb-12">
        @csrf
        ...
        <!-- Section 3: Info Unit Pelanggan & Charger -->
        <div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-100 bg-slate-50/50">
                <h2 class="text-sm font-semibold text-slate-800 uppercase tracking-wider">Unit & Charger Info</h2>
            </div>
            ...
            <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-6">
                ...
                <!-- KOLOM KANAN: Data Spesifik Charger -->
                <div class="space-y-5">
                    <h3 class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Informasi Charger</h3>
                    <div>
                        <label for="sn_charger" class="block text-xs font-medium text-slate-700 mb-1">Charger Serial
                            Number (S/N) <span class="text-red-500">*</span></label>
                        <input type="text" name="sn_charger" id="sn_charger" value="{{ old('sn_charger') }}"
                            placeholder="S/N Charger Fisik..." required
                            class="w-full px-4 py-2.5 bg-white border border-slate-300 rounded-xl text-sm text-slate-900 focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition-shadow">
                    </div>
                    <div>
                        <label for="charger_type" class="block text-xs font-medium text-slate-700 mb-1">Charger Type /
                            Model <span class="text-red-500">*</span></label>
                        <input type="text" name="charger_type" id="charger_type" value="{{ old('charger_type') }}"
                            placeholder="Tipe Model Charger" required
                            class="w-full px-4 py-2.5 bg-white border border-slate-300 rounded-xl text-sm text-slate-900 focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition-shadow">
                    </div>
                    <div>
                        <label for="charger_year" class="block text-xs font-medium text-slate-700 mb-1">Tahun Pembuatan
                            Charger</label>
                        <input type="number" name="charger_year" id="charger_year" value="{{ old('charger_year') }}"
                ...
                <div class="space-y-5">
                    <div>
                        ...
                        <select name="category_job" id="category_job" required
                            class="w-full px-4 py-2.5 bg-white border border-slate-300 rounded-xl text-sm text-slate-900 focus:ring-2 focus:ring-amber-500 focus:border-amber-500 transition-shadow">
                            <option value="CEK CHARGER" {{ old('category_job')=='CEK CHARGER' ? 'selected' : '' }}>CEK
                                CHARGER</option>
                            <option value="TARIK CHARGER" {{ old('category_job')=='TARIK CHARGER' ? 'selected' : '' }}>
                                TARIK CHARGER</option>
                            <option value="KIRIM CHARGER" {{ old('category_job')=='KIRIM CHARGER' ? 'selected' : '' }}>
                                KIRIM CHARGER</option>
                        </select>
                    </div>
                    <div>
                    ...
                    <div class="flex flex-wrap gap-2" id="job-types-container">
                        ...
                        <div class="relative group">
                            <input type="checkbox" name="job_types[]" value="{{ $type }}" id="job_{{ $loop->index }}"
                                class="peer absolute opacity-0 w-full h-full cursor-pointer z-20" {{ in_array($type,
                                $oldJobTypes) ? 'checked' : '' }}>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label for="problem" class="block text-xs font-medium text-slate-700 mb-1">Problem /
                            Temuan</label>
                        <textarea name="problem" id="problem" rows="4" placeholder="Jelaskan kendala charger..."
                            class="w-full px-4 py-3 bg-white border border-slate-300 rounded-xl text-sm text-slate-900 focus:ring-2 focus:ring-amber-500 focus:border-amber-500">{{ old('problem') }}</textarea>
                    </div>
                    <div>
                        <label for="action" class="block text-xs font-medium text-slate-700 mb-1">Action /
                            Tindakan</label>
                        <textarea name="action" id="action" rows="4" placeholder="Tindakan yang telah dilakukan..."
                            class="w-full px-4 py-3 bg-white border border-slate-300 rounded-xl text-sm text-slate-900 focus:ring-2 focus:ring-amber-500 focus:border-amber-500">{{ old('action') }}</textarea>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="flex justify-end pt-4">
            <button type="submit" id="btn-submit"
                ...
                <span id="btn-text">Simpan Data Charger</span>
            </button>
        </div>
    </form>
</div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-4">
            <div class="lg:col-span-3">
                ...
                <input type="text" name="inst_part_number[]"
                    class="w-full px-3 py-2 bg-white border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-amber-500 focus:border-amber-500">
            </div>
...
<template id="tmpl-rec">
    <div
        class="rec-item relative bg-slate-50 border border-slate-200 rounded-2xl p-4 sm:p-5 pt-8 sm:pt-5 transition-all">
        <button type="button"
            class="btn-remove-rec absolute top-2 right-2 sm:top-4 sm:right-4 p-1.5 text-slate-500 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16">
                </path>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // --- 3. MENCEGAH DOUBLE SUBMIT & VALIDASI TIPE PEKERJAAN ---
        ...
        snInput.addEventListener('input', function() {
            ...
            timeoutId = setTimeout(() => {
                fetch(`/chargers/search-assets?q=${encodeURIComponent(query)}`)
                    .then(res => res.json())
                    .then(data => {
                        ...
                        if (data.length > 0) {
                            data.forEach(item => {
                                ...
                                div.className = 'px-4 py-3 hover:bg-amber-50 cursor-pointer text-sm text-slate-700 transition-colors flex flex-col gap-1';
                                div.innerHTML = `
        ...
        // --- 5. LOGIC DYNAMIC ROWS (INSTALL & REKOMENDASI) ---
