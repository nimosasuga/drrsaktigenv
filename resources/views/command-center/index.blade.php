...
<div class="mx-auto max-w-7xl pb-28">
    <div class="mb-6 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        ...
        <form method="GET" action="{{ route('command-center.index') }}" class="flex items-center gap-2 rounded-3xl border border-slate-200 bg-white p-2 shadow-sm">
            <select name="year" class="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm font-black text-slate-700 outline-none" onchange="this.form.submit()">
                @foreach($years as $year)
                    <option value="{{ $year }}" {{ (int) $selectedYear === (int) $year ? 'selected' : '' }}>{{ $year }}</option>
                @endforeach
            </select>
            <button type="submit" class="rounded-2xl bg-blue-600 px-4 py-2.5 text-sm font-black text-white shadow-lg shadow-blue-900/20">Filter</button>
        </form>
    </div>
    ...
    <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <div class="rounded-3xl border border-blue-100 bg-white p-5 shadow-sm">
            <p class="text-xs font-black uppercase tracking-wider text-slate-400">Total Aktivitas {{ $selectedYear }}</p>
    ...
    <div class="mt-5 grid gap-5 xl:grid-cols-3">
        <section class="rounded-3xl border border-slate-100 bg-white p-5 shadow-sm xl:col-span-2">
            <div class="mb-5 flex items-center justify-between gap-3">
                <div>
                    ...
                    <p class="mt-1 text-xs font-bold text-slate-500">Gabungan seluruh modul pada tahun {{ $selectedYear }}.</p>
                </div>
                <span class="rounded-full bg-blue-50 px-3 py-1 text-xs font-black text-blue-700">Peak: {{ number_format($summary['peak_month_total']) }}</span>
            </div>
            ...
            <div class="grid grid-cols-12 items-end gap-2 rounded-3xl bg-slate-50 p-4">
                @foreach($monthLabels as $month => $label)
                    ...
                    <div class="flex flex-col items-center gap-2">
                        <div class="flex h-44 w-full items-end justify-center rounded-2xl bg-white p-1 shadow-inner" title="{{ $label }}: {{ $value }} aktivitas">
        ...
        <section class="rounded-3xl border border-slate-100 bg-white p-5 shadow-sm">
            ...
            <div class="space-y-3">
                ...
                @empty
                    <div class="rounded-2xl border border-dashed border-slate-200 p-4 text-sm font-bold text-slate-500">Belum ada data PIC pada tahun ini.</div>
                @endforelse
            </div>
        </section>
    </div>
    ...
    <section class="mt-5 rounded-3xl border border-slate-100 bg-white p-5 shadow-sm">
        <div class="mb-5">
            ...
            <p class="mt-1 text-xs font-bold text-slate-500">Format saat ini CSV UTF-8 yang bisa dibuka langsung di Excel. Import bersifat insert-only, tidak overwrite data lama.</p>
        </div>
                ...
                <div class="rounded-3xl border border-slate-100 bg-slate-50 p-4">
                    <div class="flex items-start justify-between gap-3">
                        <div>
                            ...
                            <p class="mt-1 text-xs font-bold text-slate-500">{{ $stat['table'] }} · {{ number_format($stat['year_total']) }} data tahun {{ $selectedYear }}</p>
                        </div>
                        <a href="{{ route($stat['route']) }}" class="rounded-xl bg-white px-3 py-1.5 text-xs font-black text-slate-600 shadow-sm">Buka</a>
                    </div>

                    <div class="mt-4 grid grid-cols-2 gap-2">
                        <a href="{{ route('command-center.export', $stat['key']) }}" class="rounded-2xl bg-blue-600 px-4 py-2.5 text-center text-xs font-black text-white shadow-lg shadow-blue-900/20">Export CSV</a>
                        <button type="button" onclick="document.getElementById('import-{{ $stat['key'] }}').classList.toggle('hidden')" class="rounded-2xl border border-slate-200 bg-white px-4 py-2.5 text-xs font-black text-slate-700">Import</button>
                    </div>

                    <form id="import-{{ $stat['key'] }}" action="{{ route('command-center.import', $stat['key']) }}" method="POST" enctype="multipart/form-data" class="mt-3 hidden space-y-3 rounded-2xl border border-amber-200 bg-amber-50 p-3">
                        @csrf
                        <p class="text-xs font-bold leading-relaxed text-amber-800">Gunakan file CSV hasil export sebagai template. Jangan edit kolom header sembarangan.</p>
                        <input type="file" name="file" accept=".csv,.txt" required class="block w-full text-xs font-bold text-slate-700 file:mr-3 file:rounded-xl file:border-0 file:bg-white file:px-3 file:py-2 file:text-xs file:font-black file:text-slate-700">
                        <button type="submit" onclick="return confirm('Import akan menambah data baru. Tidak ada overwrite otomatis. Lanjutkan?')" class="w-full rounded-2xl bg-amber-600 px-4 py-2.5 text-xs font-black text-white">Upload CSV</button>
                    </form>
