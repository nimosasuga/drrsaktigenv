...
<h2 class="text-lg font-bold text-slate-800 mb-4 px-1">Status Preventive Maintenance Bulan Ini</h2>

<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
    <div
        class="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm ring-1 ring-slate-900/5 lg:col-span-1">
        <div class="flex items-start justify-between gap-4">
            <div>
                <p class="text-sm font-medium text-slate-500">Progress PM</p>
                <p class="mt-2 text-3xl font-black text-slate-900">{{ $pmCompletionRate }}%</p>
                <p class="mt-1 text-xs font-semibold text-slate-500">
                    {{ number_format($pmDoneAssets, 0, ',', '.') }} dari
                    {{ number_format($pmEligibleAssets, 0, ',', '.') }} unit aktif
                </p>
            </div>
            <div class="p-3 rounded-xl bg-cyan-50 text-cyan-700">
                <svg class="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M9 17v-6a2 2 0 012-2h8m0 0l-4-4m4 4l-4 4M5 19h14"></path>
                </svg>
            </div>
        </div>
        <div class="mt-5 h-3 overflow-hidden rounded-full bg-slate-100">
            <div class="h-full rounded-full bg-cyan-600" style="width: {{ $pmCompletionRate }}%"></div>
        </div>
        <p class="mt-3 text-[11px] font-semibold uppercase tracking-wider text-slate-400">
            Periode {{ now()->translatedFormat('F Y') }}
        </p>
        <a href="{{ route('dashboard.pm-status', 'all') }}"
            class="mt-4 inline-flex w-full items-center justify-center rounded-2xl border border-cyan-100 bg-cyan-50 px-4 py-2.5 text-sm font-black text-cyan-700 transition hover:bg-cyan-100 focus:outline-none focus:ring-4 focus:ring-cyan-100">
            Lihat Detail PM
        </a>
    </div>

    <a href="{{ route('dashboard.pm-status', 'done') }}"
        class="group block bg-white rounded-2xl border border-slate-200 p-6 shadow-sm hover:shadow-md transition-shadow ring-1 ring-slate-900/5 focus:outline-none focus:ring-4 focus:ring-emerald-100">
        <div class="flex items-center">
            <div class="p-3 rounded-xl bg-emerald-50 text-emerald-600">
                <svg class="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M9 12l2 2 4-4m5.25-3.75A9 9 0 1112 3v9l6.36-6.36"></path>
                </svg>
            </div>
            <div class="ml-4">
        ...
        <span
            class="mt-4 inline-flex w-full items-center justify-center rounded-2xl border border-emerald-100 bg-emerald-50 px-4 py-2.5 text-sm font-black text-emerald-700 transition group-hover:bg-emerald-100">
            Lihat Detail
        </span>
    </a>

    <a href="{{ route('dashboard.pm-status', 'pending') }}"
        class="group block bg-white rounded-2xl border border-slate-200 p-6 shadow-sm hover:shadow-md transition-shadow ring-1 ring-slate-900/5 focus:outline-none focus:ring-4 focus:ring-rose-100">
        <div class="flex items-center">
        ...
        <span
            class="mt-4 inline-flex w-full items-center justify-center rounded-2xl border border-rose-100 bg-rose-50 px-4 py-2.5 text-sm font-black text-rose-700 transition group-hover:bg-rose-100">
            Lihat Detail
        </span>
    </a>
</div>
