<!-- resources/views/batteries/show.blade.php -->
@extends('layouts.app')
...
$canEdit = ($user->id === $battery->user_id) || in_array($role, $privilegedRoles);
@endphp
...
<div class="max-w-6xl mx-auto">
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <div class="flex items-center gap-3">
                <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Detail Pekerjaan Battery</h1>
                @if($battery->status_unit === 'RFU')
                <span
                    class="px-2.5 py-1 rounded-lg text-xs font-bold bg-emerald-100 text-emerald-700 uppercase tracking-wider">RFU</span>
                @elseif($battery->status_unit === 'BREAKDOWN')
                <span
                    class="px-2.5 py-1 rounded-lg text-xs font-bold bg-red-100 text-red-700 uppercase tracking-wider">B/D</span>
                @else
                <span
                    class="px-2.5 py-1 rounded-lg text-xs font-bold bg-amber-100 text-amber-700 uppercase tracking-wider">{{
                    $battery->status_unit ?? 'MONITORING' }}</span>
                @endif
            </div>
            <p class="text-sm text-slate-500 mt-1">ID Record: #{{ str_pad($battery->id, 5, '0', STR_PAD_LEFT) }} &bull;
                Unit S/N: <span class="font-bold text-slate-700">{{ $battery->serial_number }}</span></p>
        </div>
        ...
        <div class="flex flex-wrap items-center gap-2">
            ...
            @if($canEdit)
            <!-- Tombol Hapus -->
            <form action="{{ route('batteries.destroy', $battery->id) }}" method="POST" class="inline-block"
                onsubmit="return confirm('PERINGATAN: Yakin ingin menghapus permanen data Battery ini beserta parts di dalamnya? Aksi ini tidak dapat dibatalkan.');">
                @csrf
                @method('DELETE')
                <button type="submit"
            ...
            <!-- Tombol Edit Aktif -->
                <a href="{{ route('batteries.edit', $battery->id) }}" class="inline-flex items-center justify-center px-4 py-2 bg-emerald-600 text-white rounded-xl text-sm font-semibold hover:bg-emerald-700 transition-colors shadow-sm focus:ring-2 focus:ring-emerald-100">
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <!-- KOLOM KIRI (Info Utama & Battery) -->
        <div class="xl:col-span-2 space-y-6">

            <!-- Card 1: Informasi Unit & Battery -->
                ...
                <div class="p-6">
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
                        <div class="col-span-2">
                            ...
                            <p class="text-sm font-bold text-slate-900">{{ $battery->customer }}</p>
                        </div>
                        <div class="col-span-2">
                            <p class="text-[11px] font-semibold text-slate-400 uppercase tracking-wider mb-1">Lokasi /
                                Site</p>
                            <p class="text-sm font-bold text-slate-900">{{ $battery->location }}</p>
                        </div>

                        <div>
                            <p class="text-[11px] font-semibold text-slate-400 uppercase tracking-wider mb-1">Tipe Unit
                            </p>
                            <p class="text-sm font-medium text-slate-800">{{ $battery->unit_type }}</p>
                        </div>
                        <div class="col-span-2 md:col-span-3">
                            <p class="text-[11px] font-semibold text-slate-400 uppercase tracking-wider mb-1">Tipe
                                Pekerjaan (Job Type)</p>
                            <div class="flex flex-wrap gap-1.5 mt-0.5">
                                @foreach(explode(',', $battery->job_type) as $jtype)
                                @if(trim($jtype) != '')
                                <span
                                    class="bg-emerald-50 text-emerald-700 border border-emerald-200 px-2 py-0.5 rounded text-xs font-bold">{{
                                    trim($jtype) }}</span>
                                @endif
                                @endforeach
                            </div>
                            ...
                            <div
                                ...
                                <div>
                                    <p class="text-[11px] font-semibold text-slate-400 uppercase tracking-wider mb-1">
                                        S/N Battery Fisik</p>
                                    <p class="text-base font-bold text-emerald-700 font-mono">{{ $battery->sn_battery ??
                                        '-' }}</p>
                                </div>
                                <div>
                                    <p class="text-[11px] font-semibold text-slate-400 uppercase tracking-wider mb-1">
                                        Type / Model</p>
                                    <p class="text-sm font-bold text-slate-800">{{ $battery->battery_type ?? '-' }}</p>
                                </div>
                                <div>
                                    <p class="text-[11px] font-semibold text-slate-400 uppercase tracking-wider mb-1">
                                        Tahun Battery</p>
                                    <p class="text-sm font-bold text-slate-800">{{ $battery->battery_year ?? '-' }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <!-- Card 2: Temuan & Tindakan (Problem & Action) -->
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="bg-slate-50 p-3 rounded-xl border border-slate-100">
                            <p class="text-[11px] font-semibold text-slate-400 uppercase tracking-wider mb-1">Tgl
                                Problem / Tarik</p>
                            <p class="text-sm font-medium text-slate-800">{{ $battery->problem_date ?
                                \Carbon\Carbon::parse($battery->problem_date)->translatedFormat('d M Y') : '-' }}</p>
                        </div>
                        <div class="bg-emerald-50 p-3 rounded-xl border border-emerald-100">
                            <p class="text-[11px] font-semibold text-emerald-600 uppercase tracking-wider mb-1">Tgl RFU
                                / Selesai</p>
                            <p class="text-sm font-bold text-emerald-800">{{ $battery->rfu_date ?
                                \Carbon\Carbon::parse($battery->rfu_date)->translatedFormat('d M Y') : '-' }}</p>
                        </div>
                    </div>

                    <div>
                        <p class="text-xs font-bold text-slate-800 mb-2">Problem / Temuan:</p>
                        <div class="bg-red-50/50 p-4 rounded-2xl border border-red-100">
                            <p class="text-sm text-slate-700 whitespace-pre-line">{{ $battery->problem ?? '-' }}</p>
                        </div>
                    </div>
            ...
            <!-- Card 3: Parts Terpasang (Install Parts) -->
            <div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
                <div class="px-6 py-4 border-b border-slate-50 bg-slate-50/50 flex items-center justify-between">
                    <h2 class="text-sm font-semibold text-slate-800 uppercase tracking-wider flex items-center">
                        ...
                        Parts Terpasang ({{ $battery->installParts->count() }})
                    </h2>
                </div>
                <div class="p-6">
                    @if($battery->installParts->count() > 0)
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        @foreach($battery->installParts as $part)
                        <div
                    ...
                    <div class="text-center py-6 bg-slate-50 rounded-2xl border border-dashed border-slate-200">
                        <p class="text-sm text-slate-400">Tidak ada part yang dipasang pada pekerjaan ini.</p>
                    </div>
                    @endif
                </div>
            </div>
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
                <div class="px-6 py-4 border-b border-amber-100 bg-amber-50/50 flex items-center justify-between">
                    <h2 class="text-sm font-semibold text-amber-900 uppercase tracking-wider flex items-center">
                        ...
                        Rekomendasi Part ({{ $battery->recommendations->count() }})
                    </h2>
                </div>
                <div class="p-6">
                    @if($battery->recommendations->count() > 0)
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        @foreach($battery->recommendations as $rec)
                        <div
        ...
        <!-- KOLOM KANAN (Sidebar Info Mekanik & Kendaraan) -->
        <div class="space-y-6">
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden sticky top-6">
                ...
                <div class="p-6 flex flex-col gap-5">
                    ...
                    <div class="flex items-center gap-4">
                        <div
                            class="w-12 h-12 rounded-full bg-emerald-100 flex items-center justify-center text-emerald-700 font-bold text-lg shrink-0 border border-emerald-200">
                            {{ substr($battery->pic, 0, 1) }}
                        </div>
                        <div>
                            <p class="text-sm font-bold text-slate-900">{{ $battery->pic }}</p>
                            <p class="text-xs text-slate-500 capitalize">{{ str_replace('_', ' ',
                                $battery->status_mekanik) }}</p>
                        </div>
                    </div>
                    ...
                    <div>
                        <p class="text-[11px] font-semibold text-slate-400 uppercase tracking-wider mb-1">Branch /
                            Cabang</p>
                        <p class="text-sm font-medium text-slate-800">{{ $battery->branch }}</p>
                    </div>
                    ...
                    <div class="pt-4 border-t border-slate-100 bg-slate-50 -mx-6 px-6 py-4 mt-2">
                        ...
                        <div
                            ...
                            <div class="text-center w-full">
                                ...
                                <p class="text-sm font-bold text-slate-800">{{ $battery->date ?
                                    \Carbon\Carbon::parse($battery->date)->translatedFormat('d M Y') : '-' }}</p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-between mb-4">
                            <div class="text-center flex-1">
                                ...
                                <p class="text-sm font-bold text-emerald-600">{{ $battery->in_time ?
                                    \Carbon\Carbon::parse($battery->in_time)->format('H:i') : '-' }}</p>
                            </div>
                            <div class="w-px h-8 bg-slate-200"></div>
                            <div class="text-center flex-1">
                                <p class="text-[10px] text-slate-400 uppercase font-bold mb-0.5">Jam Keluar</p>
                                <p class="text-sm font-bold text-red-600">{{ $battery->out_time ?
                                    \Carbon\Carbon::parse($battery->out_time)->format('H:i') : '-' }}</p>
                            </div>
                        </div>

                        <div class="flex items-start justify-between border-t border-slate-200 pt-3">
                            <div>
                                <p class="text-xs text-slate-500 mb-0.5">Nama Mobil</p>
                                <p class="text-sm font-bold text-slate-800">{{ $battery->vehicle ?? '-' }}</p>
                            </div>
