...
<div class="mx-auto max-w-7xl space-y-6">
    <div class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
                ...
                <h1 class="mt-2 text-2xl font-black tracking-tight text-slate-950 sm:text-3xl">Dashboard Stok Sparepart
                    Rental</h1>
                <p class="mt-2 max-w-3xl text-sm leading-6 text-slate-500">
                    Monitoring stok aktif sparepart RENTAL berdasarkan part, lokasi penyimpanan, no job, customer, dan
                    alokasi unit.
            ...
            <div class="flex flex-col gap-2 sm:flex-row lg:flex-col">
                ...
                <a href="{{ route('rental-spareparts.reviews.index') }}"
                    class="inline-flex items-center justify-center rounded-2xl bg-purple-600 px-5 py-3 text-sm font-black text-white shadow-sm transition hover:bg-purple-700">
                    Review Usage
                </a>

                <a href="{{ route('rental-spareparts.movements.index') }}"
                    class="inline-flex items-center justify-center rounded-2xl bg-slate-900 px-5 py-3 text-sm font-black text-white shadow-sm transition hover:bg-slate-800">
                    Histori Movement
                </a>
    ...
    <div class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        <form method="GET" action="{{ route('rental-spareparts.index') }}" class="grid gap-3 lg:grid-cols-6">
            <div class="lg:col-span-2">
                <label class="mb-1 block text-xs font-bold uppercase tracking-wide text-slate-500">Cari
                    Sparepart</label>
            ...
            <div>
                ...
                <select name="location_id"
                    ...
                    <option value="{{ $location->id }}" {{ (string) $filters['location_id']===(string) $location->id ?
            ...
            <div>
                ...
                <select name="sn_unit"
                    ...
                    <option value="{{ $snUnit }}" {{ $filters['sn_unit']===$snUnit ? 'selected' : '' }}>{{ $snUnit }}
                    </option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <select name="stock_status"
                    ...
                    <option value="MENIPIS" {{ $filters['stock_status']==='MENIPIS' ? 'selected' : '' }}>Menipis
                    </option>
                    <option value="RESERVED" {{ $filters['stock_status']==='RESERVED' ? 'selected' : '' }}>Reserved
                    </option>
            ...
            <div class="lg:col-span-2">
                ...
                <select name="no_job"
                    ...
                    <option value="{{ $noJob }}" {{ $filters['no_job']===$noJob ? 'selected' : '' }}>{{ $noJob }}
                    </option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="flex gap-2 lg:col-span-4 lg:justify-end">
                <button type="submit"
                    class="inline-flex flex-1 items-center justify-center rounded-2xl bg-blue-600 px-5 py-3 text-sm font-bold text-white transition hover:bg-blue-700 sm:flex-none">Filter</button>
                <a href="{{ route('rental-spareparts.index') }}"
                    class="inline-flex items-center justify-center rounded-2xl border border-slate-200 bg-white px-5 py-3 text-sm font-bold text-slate-700 transition hover:bg-slate-50">Reset</a>
            </div>
        </form>
    </div>
    ...
    <div class="space-y-4">
        ...
        if ($available <= 0) { $statusLabel='HABIS' ; $statusClass='bg-red-50 text-red-700 border-red-200' ; } elseif
            ($stock->qty_reserved > 0) {
            $statusLabel = 'RESERVED';
            $statusClass = 'bg-blue-50 text-blue-700 border-blue-200';
            } elseif ($minStock > 0 && $available <= $minStock) { $statusLabel='MENIPIS' ;
                $statusClass='bg-amber-50 text-amber-700 border-amber-200' ; } @endphp <div
                class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm transition hover:shadow-md">
                <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div class="min-w-0 flex-1">
                        <div class="flex flex-wrap items-center gap-2">
                            <span class="rounded-full border px-2.5 py-1 text-xs font-black {{ $statusClass }}">{{
                                $statusLabel }}</span>
                            <span class="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-bold text-slate-600">{{
                                $stock->department }}</span>
                            <span class="rounded-full bg-blue-50 px-2.5 py-1 text-xs font-bold text-blue-700">{{
                                $stock->location->location_name ?? 'Tanpa Lokasi' }}</span>
                        </div>
                        ...
                        <div class="mt-4 grid grid-cols-1 gap-3 text-sm text-slate-600 md:grid-cols-3">
                            ...
                            <div class="rounded-2xl bg-slate-50 p-3">
                                ...
                                <p class="mt-1 font-bold text-slate-800">{{ $stock->allocation_customer ?:
                                    $stock->source_customer ?: '-' }}</p>
                                <p class="text-xs text-slate-500">{{ $stock->allocation_type_unit ?:
                                    $stock->source_type_unit ?: '-' }}</p>
                            </div>
                            <div class="rounded-2xl bg-slate-50 p-3">
                                <p class="text-xs font-bold uppercase tracking-wide text-slate-400">SN Unit</p>
                                <p class="mt-1 font-bold text-slate-800">{{ $stock->allocation_sn_unit ?:
                                    $stock->source_sn_unit ?: '-' }}</p>
                            </div>
                        </div>
                    ...
                    <div class="grid grid-cols-3 gap-2 lg:w-72">
                        <div class="rounded-2xl border border-emerald-200 bg-emerald-50 p-3 text-center">
                            ...
                            <p class="mt-1 text-xl font-black text-emerald-700">{{ number_format($stock->qty_on_hand) }}
                            </p>
                        </div>
                        <div class="rounded-2xl border border-amber-200 bg-amber-50 p-3 text-center">
                            ...
                            <p class="mt-1 text-xl font-black text-amber-700">{{ number_format($stock->qty_reserved) }}
                            </p>
                        </div>
