...
<div class="mx-auto max-w-7xl space-y-6">
        ...
        <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
                ...
                <p class="mt-2 max-w-3xl text-sm leading-6 text-slate-500">
                    Daftar hasil smart matching dari Update Job Install Part. Tahap ini masih read-only; approve/reject dan final OUT dibuat pada tahap 6D.
                </p>
            </div>
    ...
    <div class="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6">
        ...
        <div class="rounded-3xl border border-blue-200 bg-blue-50 p-4 shadow-sm">
            <p class="text-xs font-bold uppercase tracking-wide text-blue-500">Pending</p>
            <p class="mt-2 text-2xl font-black text-blue-700">{{ number_format($summary['pending']) }}</p>
        </div>
        <div class="rounded-3xl border border-amber-200 bg-amber-50 p-4 shadow-sm">
            <p class="text-xs font-bold uppercase tracking-wide text-amber-500">Need Source</p>
            <p class="mt-2 text-2xl font-black text-amber-700">{{ number_format($summary['need_source']) }}</p>
        </div>
        <div class="rounded-3xl border border-orange-200 bg-orange-50 p-4 shadow-sm">
            <p class="text-xs font-bold uppercase tracking-wide text-orange-500">Pinjam</p>
            <p class="mt-2 text-2xl font-black text-orange-700">{{ number_format($summary['borrowed']) }}</p>
        </div>
        <div class="rounded-3xl border border-emerald-200 bg-emerald-50 p-4 shadow-sm">
            <p class="text-xs font-bold uppercase tracking-wide text-emerald-500">Approved</p>
            <p class="mt-2 text-2xl font-black text-emerald-700">{{ number_format($summary['approved']) }}</p>
        </div>
        <div class="rounded-3xl border border-red-200 bg-red-50 p-4 shadow-sm">
            <p class="text-xs font-bold uppercase tracking-wide text-red-500">Rejected</p>
            <p class="mt-2 text-2xl font-black text-red-700">{{ number_format($summary['rejected']) }}</p>
        </div>
    </div>
    ...
    <div class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        <form method="GET" action="{{ route('rental-spareparts.reviews.index') }}" class="grid gap-3 lg:grid-cols-8">
            ...
            <div>
                ...
                <input type="text" name="part_number" value="{{ $filters['part_number'] }}" placeholder="Part number" class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm uppercase focus:border-purple-500 focus:outline-none focus:ring-4 focus:ring-purple-100">
            </div>

            <div>
                <label class="mb-1 block text-xs font-bold uppercase tracking-wide text-slate-500">SN Unit</label>
                <input type="text" name="sn_unit" value="{{ $filters['sn_unit'] }}" placeholder="Serial number" class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm uppercase focus:border-purple-500 focus:outline-none focus:ring-4 focus:ring-purple-100">
            </div>

            <div>
                <label class="mb-1 block text-xs font-bold uppercase tracking-wide text-slate-500">No Job</label>
                <input type="text" name="no_job" value="{{ $filters['no_job'] }}" placeholder="No job" class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm uppercase focus:border-purple-500 focus:outline-none focus:ring-4 focus:ring-purple-100">
            </div>
            ...
            <div>
                <label class="mb-1 block text-xs font-bold uppercase tracking-wide text-slate-500">Dari</label>
                <input type="date" name="date_from" value="{{ $filters['date_from'] }}" class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm focus:border-purple-500 focus:outline-none focus:ring-4 focus:ring-purple-100">
            </div>

            <div>
                <label class="mb-1 block text-xs font-bold uppercase tracking-wide text-slate-500">Sampai</label>
                <input type="date" name="date_to" value="{{ $filters['date_to'] }}" class="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm focus:border-purple-500 focus:outline-none focus:ring-4 focus:ring-purple-100">
            </div>

            <div class="flex gap-2 lg:col-span-8 lg:justify-end">
    ...
    <div class="space-y-4">
                    ...
                    <div class="min-w-0 flex-1">
                        <div class="flex flex-wrap items-center gap-2">
                            ...
                            @if($review->is_borrowed)
                                <span class="rounded-full border border-orange-200 bg-orange-50 px-2.5 py-1 text-xs font-black text-orange-700">PINJAM</span>
                            @endif
                        ...
                        <div class="mt-4 grid grid-cols-1 gap-3 text-sm text-slate-600 md:grid-cols-3">
                            <div class="rounded-2xl bg-slate-50 p-3">
                                <p class="text-xs font-bold uppercase tracking-wide text-slate-400">Update Job</p>
                                <p class="mt-1 font-bold text-slate-800">SN: {{ $review->job_serial_number ?: '-' }}</p>
                                <p class="text-xs text-slate-500">{{ $review->job_customer ?: '-' }}</p>
                                <p class="text-xs text-slate-500">{{ $review->job_location ?: '-' }}</p>
                                <p class="text-xs text-slate-500">No Job: {{ $review->no_job ?: '-' }}</p>
                            </div>
                            <div class="rounded-2xl bg-slate-50 p-3">
                                <p class="text-xs font-bold uppercase tracking-wide text-slate-400">Source / Alokasi Stok</p>
                                <p class="mt-1 font-bold text-slate-800">{{ $review->original_allocation_customer ?: '-' }}</p>
                                <p class="text-xs text-slate-500">{{ $review->original_allocation_location ?: '-' }}</p>
                                <p class="text-xs text-slate-500">{{ $review->original_allocation_type_unit ?: '-' }} / {{ $review->original_allocation_sn_unit ?: '-' }}</p>
                            </div>
                            <div class="rounded-2xl bg-slate-50 p-3">
                                <p class="text-xs font-bold uppercase tracking-wide text-slate-400">Actual Pemakaian</p>
                                <p class="mt-1 font-bold text-slate-800">{{ $review->actual_customer ?: '-' }}</p>
                                <p class="text-xs text-slate-500">{{ $review->actual_location ?: '-' }}</p>
                                <p class="text-xs text-slate-500">{{ $review->actual_type_unit ?: '-' }} / {{ $review->actual_sn_unit ?: '-' }}</p>
                            </div>
                        </div>

                        <div class="mt-4 grid grid-cols-1 gap-3 text-sm text-slate-600 md:grid-cols-3">
                            <div class="rounded-2xl bg-purple-50 p-3">
                                <p class="text-xs font-bold uppercase tracking-wide text-purple-400">Qty Request</p>
                                <p class="mt-1 text-xl font-black text-purple-700">{{ number_format($review->qty_requested) }}</p>
                            </div>
                            <div class="rounded-2xl bg-slate-50 p-3">
                                <p class="text-xs font-bold uppercase tracking-wide text-slate-400">Mekanik</p>
                                <p class="mt-1 font-bold text-slate-800">{{ $review->mechanic_name ?: optional($review->mechanic)->name ?: '-' }}</p>
                            </div>
                            <div class="rounded-2xl bg-slate-50 p-3">
                                <p class="text-xs font-bold uppercase tracking-wide text-slate-400">Stock Ref</p>
                                <p class="mt-1 font-bold text-slate-800">{{ optional(optional($review->stock)->item)->part_number ?: '-' }}</p>
                                <p class="text-xs text-slate-500">{{ optional(optional($review->stock)->location)->location_name ?: '-' }}</p>
                            </div>
                        </div>
        ...
        @empty
            <div class="rounded-3xl border border-dashed border-slate-300 bg-white p-10 text-center shadow-sm">
                <p class="text-lg font-black text-slate-800">Belum ada usage review.</p>
                <p class="mt-2 text-sm text-slate-500">Data akan muncul setelah Update Job RENTAL menyimpan Install Part.</p>
            </div>
        @endforelse
    </div>
