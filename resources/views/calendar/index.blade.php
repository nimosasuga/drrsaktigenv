        ...
        <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
                ...
                <p class="mt-2 max-w-3xl text-sm leading-6 text-slate-500">
                    Jadwal planning kerja untuk mekanik dan partner. Form planning dibuat ringkas agar koordinator cepat
    ...
    <div class="grid grid-cols-1 gap-6 xl:grid-cols-3">
        <div class="space-y-6 xl:col-span-1">
            ...
            <div class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
                ...
                <p class="mt-1 text-xs text-slate-500">Kolom utama: tanggal, mekanik, partner, customer, lokasi, jenis
                    pekerjaan, dan catatan.</p>

                <form method="POST" action="{{ route('calendar.plannings.store') }}" class="mt-4 space-y-4"
                    x-data="calendarPlanningForm({{ Js::from($customerLocations) }})">
                    @csrf

                    <div>
                        <label class="text-xs font-bold text-slate-500">Tanggal <span
                                class="text-red-500">*</span></label>
                        <input type="date" name="planned_date" value="{{ old('planned_date', now()->toDateString()) }}"
                            required class="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2 text-sm">
                    </div>
                    ...
                    <div>
                        <label class="text-xs font-bold text-slate-500">Customer <span
                                class="text-red-500">*</span></label>
                        <select name="customer" x-model="selectedCustomer" required
                            class="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2 text-sm">
                    ...
                    <div>
                        <label class="text-xs font-bold text-slate-500">Lokasi <span
                                class="text-red-500">*</span></label>
                        <select name="location" x-model="selectedLocation" required
                            class="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2 text-sm">
                    ...
                    <div>
                        <label class="text-xs font-bold text-slate-500">Jenis Pekerjaan <span
                                class="text-red-500">*</span></label>
                        <select name="job_type" required
                            class="mt-1 w-full rounded-xl border border-slate-300 px-3 py-2 text-sm">
                            <option value="">Pilih Jenis Pekerjaan</option>
                            @foreach($jobTypes as $jobType)
                            <option value="{{ $jobType }}" {{ old('job_type')==$jobType ? 'selected' : '' }}>{{ $jobType
                                }}</option>
                            @endforeach
                        </select>
                    </div>
