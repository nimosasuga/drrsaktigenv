<!-- PATH FILE: resources/views/assets/edit.blade.php -->
@extends('layouts.app')

@section('content')
<div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between">
    <div>
        <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Edit Data Aset</h1>
        <p class="mt-1 text-sm text-slate-500">Ubah semua kolom unit asset untuk S/N: <strong class="text-slate-700">{{ $asset->serial_number }}</strong></p>
    </div>
...
<div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden ring-1 ring-slate-900/5">
    <form action="{{ route('assets.update', $asset->id) }}" method="POST">
            ...
            <div>
                ...
                <div class="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-2 lg:grid-cols-4">
                    <div>
                        ...
                        <input type="text" value="{{ $asset->id }}" readonly
                    ...
                    <div>
                        ...
                        <input type="text" value="{{ $asset->qr_token ?? '-' }}" readonly
                            class="mt-1 block w-full rounded-xl border border-slate-200 bg-slate-100 px-3 py-2 text-slate-500 sm:text-sm">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-700">Created At</label>
                        <input type="text" value="{{ $asset->created_at ? $asset->created_at->format('Y-m-d H:i:s') : '-' }}" readonly
                            class="mt-1 block w-full rounded-xl border border-slate-200 bg-slate-100 px-3 py-2 text-slate-500 sm:text-sm">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-700">Updated At</label>
                        <input type="text" value="{{ $asset->updated_at ? $asset->updated_at->format('Y-m-d H:i:s') : '-' }}" readonly
                            class="mt-1 block w-full rounded-xl border border-slate-200 bg-slate-100 px-3 py-2 text-slate-500 sm:text-sm">
                    </div>
                </div>
                <p class="mt-3 text-xs font-bold text-slate-500">Kolom sistem ditampilkan lengkap, tetapi tidak bisa diedit manual.</p>
            </div>
            ...
            <div>
                ...
                <div class="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <input type="text" name="supported_by" id="supported_by" value="{{ old('supported_by', $asset->supported_by) }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="customer" id="customer" value="{{ old('customer', $asset->customer) }}"
                            class="mt-1 block w-full rounded-xl border border-slate-300 px-3 py-2 text-slate-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 sm:text-sm transition-colors">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="branch" id="branch" value="{{ old('branch', $asset->branch) }}"
                            class="mt-1 block w-full rounded-xl border border-slate-300 px-3 py-2 text-slate-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 sm:text-sm transition-colors">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="unit_type" id="unit_type" value="{{ old('unit_type', $asset->unit_type) }}"
                            class="mt-1 block w-full rounded-xl border border-slate-300 px-3 py-2 text-slate-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 sm:text-sm transition-colors">
                    </div>

                    <div>
                        <label for="year" class="block text-sm font-medium text-slate-700">Year</label>
                        <input type="text" name="year" id="year" value="{{ old('year', $asset->year) }}"
                            class="mt-1 block w-full rounded-xl border border-slate-300 px-3 py-2 text-slate-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 sm:text-sm transition-colors">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="delivery" id="delivery" value="{{ old('delivery', $asset->delivery) }}"
                            class="mt-1 block w-full rounded-xl border border-slate-300 px-3 py-2 text-slate-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 sm:text-sm transition-colors">
                    </div>

                    <div>
                        <label for="jenis_unit" class="block text-sm font-medium text-slate-700">Jenis Unit</label>
                        <input type="text" name="jenis_unit" id="jenis_unit" value="{{ old('jenis_unit', $asset->jenis_unit) }}"
                            class="mt-1 block w-full rounded-xl border border-slate-300 px-3 py-2 text-slate-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 sm:text-sm transition-colors">
                    </div>
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <textarea name="note" id="note" rows="3"
                            class="mt-1 block w-full rounded-xl border border-slate-300 px-3 py-2 text-slate-900 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 sm:text-sm transition-colors">{{ old('note', $asset->note) }}</textarea>
                    </div>
        ...
        <div class="bg-slate-50 px-6 py-4 sm:flex sm:flex-row-reverse border-t border-slate-200">
            <button type="submit"
                class="w-full inline-flex justify-center rounded-xl border border-transparent bg-emerald-600 px-6 py-2.5 text-base font-bold text-white shadow-sm hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 sm:ml-3 sm:w-auto sm:text-sm transition-colors">
                Simpan Perubahan
            </button>
            <a href="{{ route('assets.show', $asset->id) }}"
